# -*- coding: utf-8 -*-
"""
Created on Wed Apr 10 16:34:22 2024
...
df = pd.read_csv('co2_emissions.csv', delimiter=';')
...
if user_input == 'Visulization':
    ...
    df_brand = df['make'].value_counts().reset_index().rename(columns={'make':'count'})
    df_brand = df_brand.rename(columns={'index':'make'})
    ...
    df_model = df['model'].value_counts().reset_index().rename(columns={'model':'count'})[:25]
    ...
    df_vehicle_class = df['vehicle_class'].value_counts().reset_index().rename(columns={'vehicle_class':'count'})
    df_vehicle_class = df_vehicle_class.rename(columns={'index':'vehicle_class'})
    ...
    df_engine_size = df['engine_size'].value_counts().reset_index().rename(columns={'engine_size':'count'})
    df_engine_size = df_engine_size.rename(columns={'index':'engine_size'})
    plt.figure(figsize=(20, 6))
    ...
    df_cylinders = df['cylinders'].value_counts().reset_index().rename(columns={'cylinders':'count'})
    df_cylinders = df_cylinders.rename(columns={'index':'cylinders'})
    plt.figure(figsize=(20, 6))
    ...
    df_transmission = df['transmission'].value_counts().reset_index().rename(columns={'transmission':'Count'})
    ...
    df_fuel_type = df['fuel_type'].value_counts().reset_index().rename(columns={'fuel_type':'Count'})
    df_fuel_type = df_fuel_type.rename(columns={'index':'fuel_type'})
    ...
    df_ftype = df_natural['fuel_type'].value_counts().reset_index().rename(columns={'fuel_type': 'Count'})
    fig8 = plt.figure(figsize=(20, 5))
    sns.barplot(data=df_ftype, x="index", y="Count")
    plt.title("All Fuel Types")
    plt.xlabel("fuel_type")
    plt.ylabel("Cars")
    plt.bar_label(plt.gca().containers[0])
